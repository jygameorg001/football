...
export default class Shoot extends cc.Component {
  ...
  shootPlay() {
    ...
    if (!canRunning) {
      Game.instance.showToast("服務器數據異常...");
  ...
  kuangAni(index: number) {
    ...
    //    设置位置
    this.Jinzhong.setPosition(
      this.giftList[index].getPosition().x,
      this.giftList[index].getPosition().y + 167
    );
  ...
  closeSVGA() {
    this.svga.node.active = false;
    if (this.tiemout) {
      this.unschedule(this.tiemout);
      this.tiemout = null;
    }
  }
  ...
  noShowReward() {
    ...
    if (
      ...
      GameLogic.instance.ShootingInfo.rewardList &&
