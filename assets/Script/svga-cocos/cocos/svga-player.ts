...
export default class SVGAPlayer extends cc.Component {
  ...
  setImage(
    ...
    if (this.videoItem) {
      ...
      this.contentLayer.removeAllChildren(false);
      this._addLayers();
  ...
  _doStart(range, reverse, fromFrame) {
    ...
    if (range) {
      ...
      this._animator.duration =
        this.videoItem.frames * this._animator.dt * 1000;
    }
  ...
  _playAudio(frame) {
    if (
      ...
      this._animator.reverse === false &&
      this.videoItem &&
      this.videoItem.audios instanceof Array
    ) {
  ...
  update(dt) {
    ...
    let td =
      this._animator.animatorTime -
      this._animator.animatorFrame * this._animator.dt;
    ...
    if (typeof this._onPercentage === "function") {
      let percent =
        parseFloat((this._currentFrame + 1) as any) /
        Number(this.videoItem.frames);
      if (this._animator.reverse) {
