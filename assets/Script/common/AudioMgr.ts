...
export class AudioMgr {
  ...
  static rebuildAudioContextAndReplay() {
    ...
    try {
      ...
      support.context = new (window.AudioContext ||
      ...
      if (AudioMgr.currentMusicClip) {
        AudioMgr.currentMusicId = cc.audioEngine.playMusic(
          AudioMgr.currentMusicClip,
          AudioMgr.currentMusicLoop ?? true
        );
        cc.audioEngine.setMusicVolume(AudioMgr.bgmVolume);
      }
    } catch (e) {
  ...
  static playMusic(
    ...
    cc.resources.load(path, cc.AudioClip, (err, clip: cc.AudioClip) => {
      if (!clip) {
        console.error(
          `Failed to play music: Resource ${path} could not be loaded.`
        );
        return;
      }
  ...
  static playSound(
    ...
    cc.resources.load(path, cc.AudioClip, (err, clip: cc.AudioClip) => {
      if (!clip) {
        console.error(
          `Failed to play sound: Resource ${path} could not be loaded.`
        );
        return;
      }
