...
export class BallRun {
   ...
   * @param startPoint
   * @param endPoint
   * @param type  0=>内马尔  1=>梅西
   * @returns
   */
  getControlPoint(startPoint, endPoint, type) {
    // 礼物ID%3。共三行
    let idx = this.targetIdx % 3;
    // const pts = [cc.v2(-450, 120), cc.v2(-100, 700), cc.v2(450, 150)];
    const pts = [
      cc.v2(-315, 100),
      cc.v2(type ? 128 : -128, 175),
      cc.v2(315, 100),
    ];
    let pt = pts[idx];
    let x = 0;
  ...
  runCircleNoraml(startPoint: cc.Vec2, endPoint: cc.Vec2, callback) {
    ...
    // 控制点
    const controlPoint = this.getControlPoint(startPoint, endPoint, 1);
  ...
  runCircleEasing(startPoint, endPoint, callback?) {
    ...
    //1 4 7 :(-100,900)
    //2 5 8 :(450,150)
    const controlPoint = this.getControlPoint(startPoint, endPoint, 0);
    this.drawLine(startPoint, endPoint, controlPoint);
  ...
  runLineNoraml(startPoint, endPoint, callback) {
    ...
    cc.tween(this.football)
      .to(
        dt,
        { position: cc.v3(endPoint.x, endPoint.y), scale: BallMinScale },
        { easing: "smooth" }
      )
      .call(() => {
        // callback?.();
  ...
  ballShoot2Idx(idx: number, callback) {
    ...
    let centerPoint = cc.v3(
      (this.football.x + endPoint.x) / 2 + offsetX,
      (this.football.y + endPoint.y) / 2 + idx < 3 ? 10 : offsetY
    );
    cc.tween(this.football)
      .to(0.15, { position: centerPoint, scale: BallMinScale * 1.2 })
      .to(0.15, {
        position: cc.v3(endPoint.x, endPoint.y, 0),
        scale: BallMinScale,
      })
      .call(() => {
        // callback?.();
